...
- Generic relationship records were added for broader relationship modeling without disturbing the existing party-to-party relationship expansion used by conflict search. `RelationshipTypes` seed starter values for `Entity_Entity`, `Entity_User`, and `User_User`; `EntityRelationships` stores from/to entity type and ID pairs with notes. Client and user detail pages expose add/delete relationship panels using target record number or GUID lookup.
